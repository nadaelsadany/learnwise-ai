...
export const useSmartNotifications = () => {
  ...
  // Generate smart notifications from student data
  useEffect(() => {
    if (!user) return;
      ...
      try {
        ...
        if (dueCards.length > 0) {
          const n: SmartNotification = {
            ...
            message: dueCards.length > 5
              ? `You have ${dueCards.length} flashcards waiting for review. Reviewing now prevents forgetting!`
              : `${dueCards.length} cards are ready for review. A quick session will strengthen your memory.`,
        ...
        // 2. Study goal check (< 3 sessions this week)
        if (sessions.length < 3 && now.getDay() >= 3) {
          generated.push({
            id: 'goal-unmet',
            type: 'goal_unmet',
            title: 'Weekly Study Goal at Risk',
            message: `Only ${sessions.length} study sessions this week. Try to fit in ${5 - sessions.length} more to stay on track.`,
            priority: 'high',
            read: false,
            createdAt: now.toISOString(),
            actionUrl: '/time-blocking',
            actionLabel: 'Plan Session',
          });
        }

        // 3. Focus drop - recent short sessions
        const recentSessions = sessions.slice(-5);
        const avgDuration = recentSessions.length > 0
          ? recentSessions.reduce((s, sess) => s + (sess.duration_seconds || 0), 0) / recentSessions.length
          : 0;
        if (recentSessions.length >= 3 && avgDuration < 900) {
          generated.push({
            id: 'focus-drop',
            type: 'focus_drop',
            title: 'Focus Score Dropping',
            message: 'Your recent study sessions have been short. Try the Pomodoro technique for longer, deeper focus.',
            priority: 'medium',
            read: false,
            createdAt: now.toISOString(),
            actionUrl: '/time-blocking',
        ...
        if (completionDates.has(yesterday) && !completionDates.has(today) && now.getHours() >= 18) {
          const n: SmartNotification = {
            ...
            message: "Complete one lesson or review session today to keep your streak alive!",
            priority: 'high',
            read: false,
            createdAt: now.toISOString(),
            actionUrl: '/courses',
        ...
        // 5. Morning planning nudge
        if (now.getHours() < 10 && todayBlocks.length === 0) {
          generated.push({
            id: 'morning-plan',
            type: 'study_reminder',
            title: 'Good Morning! Plan Your Day',
            message: "Start your day right — create a study schedule to maximize productivity.",
            priority: 'low',
            read: false,
            createdAt: now.toISOString(),
            actionUrl: '/time-blocking',
        ...
        if (recentQuizzes.length >= 2) {
          ...
          if (avgScore < 60) {
            generated.push({
              ...
              message: `Your recent quiz average is ${Math.round(avgScore)}%. Consider reviewing weak topics with the AI Coach.`,
              priority: 'medium',
              read: false,
              createdAt: now.toISOString(),
              actionUrl: '/ai-coach',
